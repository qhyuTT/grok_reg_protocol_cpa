...
def _get_custom_mail_pool():
    global _custom_mail_pool
    if _custom_mail_pool is None:
        _custom_mail_pool = CustomMailPool(config, (_EMAILS_USED_FILE, _EMAILS_ERROR_FILE))
    else:
        _custom_mail_pool.config = config
...
def _custom_mail_release(email):
    if _custom_mail_pool is not None:
        try:
            _custom_mail_pool.release(email)
        except Exception:
            pass
...
def save_exported_cookies_snapshot(cookies, tag: str = "", email: str = "", url: str = ""):
    """保存已经导出的 cookie；用于健康检查通过后再持久化快照。"""
...
def save_cookies_snapshot(page, tag: str = "", email: str = ""):
    ...
    try:
        ...
        if not browser:
            return
        save_exported_cookies_snapshot(
            browser.cookies(),
            tag=tag,
            email=email,
...
class PermissionDeniedRegistration(Exception):
    """A fully registered candidate whose chat permission was explicitly denied."""

    def __init__(self, email: str):
        super().__init__(f"permission_denied: {email}")
        self.email = email


class ActivationFailedRegistration(Exception):
    """A registered candidate that failed the required Grok Web activation."""

    def __init__(self, email: str, reason: str = "web_activation_failed"):
        super().__init__(f"web_activation_failed: {email}: {reason}")
        self.email = email
        self.reason = reason
...
def get_email_and_token(api_key=None):
    ...
    if provider in ("custommail", "custom_mail"):
        return _get_custom_mail_pool().allocate()
...
def get_oai_code(
    ...
    provider = get_email_provider()
    if provider in ("custommail", "custom_mail"):
        return _get_custom_mail_pool().get_code(
            dev_token,
            email,
...
def _activation_conversation_url(page):
    try:
        ...
        return ""
    if re.search(r"https?://(?:www\.)?grok\.com/c/[^/?#]+", url):
        return url
    return ""


def _activation_pick_birth_button(page):
    """Find a human-facing save/continue button inside the birthday dialog."""
    try:
        buttons = page.eles('css:[role="dialog"] button', timeout=0.15)
    except Exception:
        buttons = []
    visible = []
    for button in buttons or []:
        try:
            ...
            if any(word in text for word in GROK_BIRTH_BUTTON_WORDS):
                return button
        except Exception:
            continue
...
def _activation_packet_url(packet):
    try:
        return str(getattr(packet, "url", "") or "")
    except Exception:
        return ""


def _activation_url_is_grok(url):
    return bool(re.search(r"https?://(?:www\.)?grok\.com(?:/|$)", str(url or "")))


def _activation_auth_state(page):
    """Return authenticated/anonymous/unknown from the visible Grok shell."""
    try:
        state = page.run_js(
            r"""
const visible = (node) => {
...
const authWords = /(登录|注册|登录帐号|log\s*in|sign\s*in|sign\s*up|register)/i;
const anonymous = [...document.querySelectorAll('a,button')]
  .filter(visible)
  .map((n) => (n.innerText || n.textContent || '').replace(/\s+/g, ' ').trim())
  .some((t) => authWords.test(t));
const userMarker = !!document.querySelector(
  '[data-testid="new-chat"], [data-testid*="user"], [data-testid*="profile"], '
  + '[aria-label*="账户"], [aria-label*="account"]'
);
if (anonymous) return 'anonymous';
if (userMarker) return 'authenticated';
return 'unknown';
"""
        )
...
def _activation_session_probe(page):
    """Actively verify the already-loaded Grok session without exposing data."""
    try:
        result = page.run_js(
            r"""
return Promise.all([
  fetch('/api/auth/session', {credentials: 'include', cache: 'no-store'}),
  fetch('/rest/user-settings', {credentials: 'include', cache: 'no-store'})
]).then(async ([sessionResponse, settingsResponse]) => {
  let sessionData = null;
  try { sessionData = await sessionResponse.json(); } catch (_) {}
  const nonEmpty = !!sessionData && typeof sessionData === 'object'
    && Object.keys(sessionData).length > 0;
  const identity = !!(sessionData && (
    sessionData.user || sessionData.email || sessionData.id ||
    sessionData.sub || sessionData.account || sessionData.expires
  ));
  return {
    auth_session_status: sessionResponse.status,
    user_settings_status: settingsResponse.status,
    session_nonempty: nonEmpty,
    session_identity: identity,
  };
}).catch((error) => ({
  auth_session_status: 0,
  user_settings_status: 0,
  session_nonempty: false,
  session_identity: false,
  request_error: String(error && error.message || error || 'fetch failed').slice(0, 200),
}));
""",
            timeout=15,
        )
        return dict(result) if isinstance(result, dict) else {
            ...
            "session_nonempty": False,
            "session_identity": False,
            "request_error": "invalid session probe result",
        }
    except Exception as exc:
        return {
            "auth_session_status": 0,
            ...
            "session_nonempty": False,
            "session_identity": False,
            "request_error": str(exc)[:200],
        }


def wait_for_authenticated_grok_page(page, log_callback=None, cancel_callback=None, timeout=60):
    """Wait for Grok's natural post-registration session hydration.

    A chat box alone is not sufficient because the anonymous shell also
    exposes one.  The current Grok app consistently requests both the session
    and user-settings endpoints after the registered account is attached.
    """
    if page is None:
    ...
    try:
        while time.time() < deadline:
            ...
            if now >= next_probe_at:
                last_probe = _activation_session_probe(page)
                next_probe_at = now + 2.0

            auth_state = _activation_auth_state(page)
            endpoints_ready = (
                200 <= int(last_probe.get("auth_session_status") or 0) < 300
                and 200 <= int(last_probe.get("user_settings_status") or 0) < 300
                and bool(last_probe.get("session_nonempty"))
            )
            if endpoints_ready and auth_state != "anonymous":
                if log_callback:
                    log_callback(
                        "[activation] Grok 登录态确认完成 "
                        f"auth_session={last_probe.get('auth_session_status')} "
                        f"user_settings={last_probe.get('user_settings_status')}"
                    )
                return {
                    "ok": True,
                    "status": "authenticated",
                    **last_probe,
                    "ready_url": str(getattr(page, "url", "") or ""),
                    "dom_state": auth_state,
                }
            sleep_with_cancel(0.15, cancel_callback)

        auth_state = _activation_auth_state(page)
        reason = (
            "grok_session_not_authenticated"
            if auth_state == "anonymous"
            else "grok_session_probe_timeout"
        )
        return {
            "ok": False,
            "reason": reason,
            ...
            if sso_value:
                if not logged_sso and log_callback:
                    log_callback(
                        ...
                        + (f" domain={sso_domain}" if sso_domain else "")
                    )
                    logged_sso = True
...
def export_cpa_after_success(
    ...
    if result.get("ok") and result.get("path"):
        ...
        log("[cpa] 权限健康检查完成；CPA 文件导出已关闭")
    elif result.get("rejected"):
        log(
            "[cpa] 健康门拒绝: "
            f"{result.get('rejection_summary') or result.get('error') or result}"
        )
    else:
...
class GrokRegisterGUI:
    def __init__(self, root):
        ...
        self.activation_failed_count = 0
        self.results = []
    ...
    def setup_ui(self):
        ...
        config_container = ttk.LabelFrame(main_frame, text="配置", padding=(0, 4))
        config_container.pack(fill=tk.X, pady=5)
        config_canvas = tk.Canvas(config_container, height=460, highlightthickness=0)
        config_scrollbar = ttk.Scrollbar(
            config_container, orient=tk.VERTICAL, command=config_canvas.yview
        )
        ...
        config_window = config_canvas.create_window((0, 0), window=config_frame, anchor=tk.NW)

        def update_config_scrollregion(_event=None):
        ...
        config_frame.bind("<Configure>", update_config_scrollregion)
        ...
        self.email_provider_combo = ttk.Combobox(config_frame, textvariable=self.email_provider_var, values=["duckmail", "yyds", "cloudflare", "cloudmail", "hotmail", "outlookmail", "custommail", 'mailnest'], width=12, state="readonly")
        ...
        custom_mail_frame = ttk.LabelFrame(config_frame, text="CustomMail（域名转发到 Gmail）", padding=6)
        custom_mail_frame.grid(row=19, column=0, columnspan=4, sticky=tk.EW, pady=(8, 0))
        custom_mail_frame.columnconfigure(1, weight=1)

        ttk.Label(custom_mail_frame, text="凭证文件:").grid(row=0, column=0, sticky=tk.W)
        self.custom_mail_accounts_file_var = tk.StringVar(
            value=str(config.get("custom_mail_accounts_file", "custom_mail_credentials.txt"))
        )
        self.custom_mail_accounts_file_entry = ttk.Entry(
            custom_mail_frame, textvariable=self.custom_mail_accounts_file_var, width=34
        )
        self.custom_mail_accounts_file_entry.grid(row=0, column=1, columnspan=2, sticky=tk.EW, padx=5)
        self.custom_mail_browse_btn = ttk.Button(
            custom_mail_frame, text="选择...", command=self._browse_custom_mail_accounts_file
        )
        self.custom_mail_browse_btn.grid(row=0, column=3, sticky=tk.W)

        ttk.Label(custom_mail_frame, text="地址前缀:").grid(row=1, column=0, sticky=tk.W, pady=(5, 0))
        self.custom_mail_address_prefix_var = tk.StringVar(
            value=str(config.get("custom_mail_address_prefix", "reg"))
        )
        ttk.Entry(custom_mail_frame, textvariable=self.custom_mail_address_prefix_var, width=12).grid(
            row=1, column=1, sticky=tk.W, padx=5, pady=(5, 0)
        )
        ttk.Label(custom_mail_frame, text="每条凭证最大地址数:").grid(
            row=1, column=2, sticky=tk.E, padx=(12, 5), pady=(5, 0)
        )
        self.custom_mail_max_addresses_var = tk.StringVar(
            value=str(config.get("custom_mail_max_addresses_per_account", 100))
        )
        ttk.Spinbox(
            custom_mail_frame, from_=1, to=100000, width=8,
            textvariable=self.custom_mail_max_addresses_var,
        ).grid(row=1, column=3, sticky=tk.W, pady=(5, 0))

        custom_mail_timing_frame = ttk.Frame(custom_mail_frame)
        custom_mail_timing_frame.grid(row=2, column=0, columnspan=4, sticky=tk.W, pady=(5, 0))
        ttk.Label(custom_mail_timing_frame, text="轮询秒数:").pack(side=tk.LEFT)
        self.custom_mail_poll_interval_var = tk.StringVar(
            value=str(config.get("custom_mail_poll_interval", 5))
        )
        ttk.Entry(custom_mail_timing_frame, textvariable=self.custom_mail_poll_interval_var, width=6).pack(
            side=tk.LEFT, padx=(5, 12)
        )
        ttk.Label(custom_mail_timing_frame, text="邮件时间窗(秒):").pack(side=tk.LEFT)
        self.custom_mail_recent_seconds_var = tk.StringVar(
            value=str(config.get("custom_mail_recent_seconds", 900))
        )
        ttk.Entry(custom_mail_timing_frame, textvariable=self.custom_mail_recent_seconds_var, width=8).pack(
            side=tk.LEFT, padx=(5, 12)
        )
        ttk.Label(custom_mail_timing_frame, text="扫描邮件数:").pack(side=tk.LEFT)
        self.custom_mail_imap_last_n_var = tk.StringVar(
            value=str(config.get("custom_mail_imap_last_n", 50))
        )
        ttk.Entry(custom_mail_timing_frame, textvariable=self.custom_mail_imap_last_n_var, width=6).pack(
            side=tk.LEFT, padx=5
        )

        ttk.Label(custom_mail_frame, text="允许发件域名:").grid(row=3, column=0, sticky=tk.W, pady=(5, 0))
        self.custom_mail_allowed_sender_domains_var = tk.StringVar(
            value=str(config.get("custom_mail_allowed_sender_domains", "x.ai,grok.com"))
        )
        ttk.Entry(
            custom_mail_frame,
            textvariable=self.custom_mail_allowed_sender_domains_var,
            width=34,
        ).grid(row=3, column=1, columnspan=3, sticky=tk.EW, padx=5, pady=(5, 0))
        ...
        self.stats_var = tk.StringVar(value="成功: 0 | 失败: 0 | 权限拒绝: 0")
    ...
    def _browse_custom_mail_accounts_file(self):
        selected = filedialog.askopenfilename(
            parent=self.root,
            title="选择 CustomMail 凭证文件",
            filetypes=[("文本文件", "*.txt"), ("所有文件", "*.*")],
        )
        if selected:
            self.custom_mail_accounts_file_var.set(selected)

    @staticmethod
    def _count_valid_custom_mail_accounts(path):
        """统计可供启动的 CustomMail 记录，不读取或输出密码内容。"""
        valid_domains = set()
        valid_gmail_accounts = set()
        with open(path, "r", encoding="utf-8-sig") as f:
            for raw_line in f:
                line = raw_line.strip()
                if not line or line.startswith("#") or line.startswith("//"):
                ...
                if len(parts) != 3 or not all(parts):
                    continue
                domain, gmail_account, _app_password = parts
                domain = domain.lower().lstrip("@")
                gmail_account = gmail_account.lower()
                if "." not in domain or "@" not in gmail_account:
                    continue
                if domain in valid_domains:
                    continue
                valid_domains.add(domain)
                valid_gmail_accounts.add(gmail_account)
        return len(valid_domains), len(valid_gmail_accounts)

    def update_stats(self):
        self.stats_var.set(
            f"成功: {self.success_count} | 失败: {self.fail_count} | "
            f"权限拒绝: {self.health_rejected_count} | Web激活失败: {self.activation_failed_count}"
        )
...
- custommail: 需要自有域名通过 Cloudflare Email Routing 转发到 Gmail，并提供 CustomMail 凭证文件
...
8) CustomMail 模式配置
- 凭证文件默认: custom_mail_credentials.txt
- 每行格式: 自有域名----Gmail收件箱----Gmail应用专用密码
- Cloudflare 只负责把域名邮件转发到 Gmail，不需要在本工具填写 Cloudflare API Key
- Gmail 需启用两步验证、IMAP 和应用专用密码；不要填写 Gmail 普通登录密码
- 转发邮件必须保留原始收件地址，程序才可按注册邮箱精确匹配验证码
- 应用专用密码仅保存在凭证文件中，不会显示或写入 config.json
...
- 如果关闭了启动教程，可随时点主界面的“教程”按钮重新打开。"""
    ...
    def start_registration(self):
        ...
        config["custom_mail_accounts_file"] = (
            self.custom_mail_accounts_file_var.get().strip() or "custom_mail_credentials.txt"
        )
        config["custom_mail_address_prefix"] = self.custom_mail_address_prefix_var.get().strip() or "reg"
        config["custom_mail_allowed_sender_domains"] = (
            self.custom_mail_allowed_sender_domains_var.get().strip() or "x.ai,grok.com"
        )
        custom_mail_numeric_fields = (
            ("custom_mail_max_addresses_per_account", self.custom_mail_max_addresses_var, 1, 100000),
            ("custom_mail_poll_interval", self.custom_mail_poll_interval_var, 1, 300),
            ("custom_mail_recent_seconds", self.custom_mail_recent_seconds_var, 30, 86400),
            ("custom_mail_imap_last_n", self.custom_mail_imap_last_n_var, 1, 1000),
        )
        for key, variable, minimum, maximum in custom_mail_numeric_fields:
            try:
                config[key] = max(minimum, min(maximum, int(variable.get())))
            except Exception:
                self.log(f"[!] CustomMail 数值配置无效: {key}")
                return
        try:
        ...
        if config["email_provider"] == "custommail":
            custom_mail_path = _resolve_project_path(
                config.get("custom_mail_accounts_file"), "custom_mail_credentials.txt"
            )
            if not os.path.isfile(custom_mail_path):
                self.log(f"[!] CustomMail 模式凭证文件不存在: {custom_mail_path}")
                return
            try:
                domain_count, gmail_count = self._count_valid_custom_mail_accounts(custom_mail_path)
            except Exception as exc:
                self.log(f"[!] CustomMail 凭证文件读取失败: {exc}")
                return
            if domain_count < 1:
                self.log("[!] CustomMail 凭证文件没有有效记录，请使用: 自有域名----Gmail收件箱----Gmail应用专用密码")
                return
            self.log(
                f"[*] CustomMail 已加载 {domain_count} 个域名，关联 {gmail_count} 个 Gmail 收件箱"
            )
        try:
    ...
    def _run_single_registration(self, idx, total, logf):
        ...
        logf("[*] 6. 执行 Grok Web 首次激活")
        activation = activate_grok_web_account(
            _get_page(),
            log_callback=logf,
        ...
        if not activation.get("ok"):
            ...
            try:
                mark_error(email, password, reason=detail)
            except Exception:
                pass
            with self.stats_lock:
                self.activation_failed_count += 1
            raise ActivationFailedRegistration(email, detail)
        try:
            settle_seconds = max(
                0.0,
                float(config.get("registration_post_activation_settle_sec", 5) or 0),
            )
        except Exception:
            settle_seconds = 5.0
        if settle_seconds > 0:
            logf(f"[activation] 会话完成，等待 {settle_seconds:g}s 后执行 OIDC 健康门")
            sleep_with_cancel(settle_seconds, self.should_stop)
        result_record = {"email": email, "sso": sso, "profile": profile}
        result_record["activation"] = activation
        try:
            page = _get_page()
        except Exception:
            page = None
        ...
        if cpa_result.get("rejected"):
            rejection_summary = str(
                cpa_result.get("rejection_summary")
                or cpa_result.get("error")
                or "health_gate_rejected"
            )[:240]
            try:
                mark_error(email, password, reason=rejection_summary)
            except Exception:
                pass
            with self.stats_lock:
            ...
            while not self.should_stop_scheduling():
                ...
                try:
                    ...
                    if replacement < max_replacements and not self.should_stop_scheduling():
                        with self.stats_lock:
                            next_idx = getattr(self, "next_attempt_idx", total + 1)
                            self.next_attempt_idx = next_idx + 1
                        task_queue.put(
                            {
                                "idx": next_idx,
                                "slot": slot,
